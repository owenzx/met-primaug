...
class TransformerEncDec(nn.Module):
    def __init__(self,
        ...
        self.model, output_dim, self.encoder_embeddings, self.decoder_embeddings= get_onmt_transformer_seq2seq_model(vocab_x, vocab_y, transformer_config)
    ...
    def forward(self, inp, out, lens=None, per_instance=False, additional_output=False, use_updated_emb=False, detach_mask=None):
        if (use_updated_emb is True) or (detach_mask is not None):
            model_outputs = self.model.advanced_forward(inp.unsqueeze(-1), out.unsqueeze(-1), lens, use_updated_emb=use_updated_emb, detach_mask=detach_mask, additional_output=additional_output)
        else:
            model_outputs = self.model(inp.unsqueeze(-1), out.unsqueeze(-1), lens, additional_output=additional_output)

        if additional_output:
            additional_output = model_outputs[-1]

        dec_out = model_outputs[0]
        logits = self.model.generator(dec_out)
        if per_instance:
            out_tgt = out[1:, :].transpose(0,1)
            output = logits.permute(1,2,0)
            # loss = self.nll_wr(output,out_tgt).sum(dim=-1)
            per_step_loss = self.nll_wr(output,out_tgt)
            if additional_output:
                additional_output['per_step_loss'] = per_step_loss
            loss = per_step_loss.sum(dim=-1)
        else:
        ...
        if additional_output:
            #TODO also feed the log probs into add_dict
